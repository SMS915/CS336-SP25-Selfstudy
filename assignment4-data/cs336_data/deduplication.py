...
def generate_signature_for_texts(
...
def find_true_duplicate_pair(candidate_pairs: Set[tuple[int,...]], id_paths_map: Dict[int, os.PathLike],
                             n: int, jaccard_threshold: float = 0.8, num_workers: int = 30) -> List[Tuple[int, int]]:
    """
...
def minhash_deduplication(
    input_files: List[os.PathLike], 
    input_base_dir: str | os.PathLike,
    num_hashes: int, 
    num_bands: int, 
    n: int, 
    ...
    # 精确 Jaccard 验证
    duplicate_pairs = find_true_duplicate_pair(candidate_pairs, id_paths_map, n, jaccard_threshold, max_workers)
    ...
    print(f"正在保存去重后的唯一文件至: {output_dir}")
    for doc_id in tqdm(doc_id_to_keep, desc="文件同步"):
        input_file_path = Path(id_paths_map[doc_id])
        try:
            # 计算相对路径以保持原有的 WET 子目录结构
            relative_path = input_file_path.relative_to(input_base_path)
            output_file_path = output_base_path / relative_path
        except ValueError:
            # 防御性编程：如果文件不在 base_dir 下，直接取文件名
            output_file_path = output_base_path / input_file_path.name
    ...
    return final_output_paths, total_docs, unique_docs
