...
def build_positive_data(warc_file_path: str, quality_label: str, fasttext_format_str: str, max_sample: Optional[int] = None) -> tuple[list[str], int]:
    ...
    with open(warc_file_path, 'rb') as warc_file:
        with tqdm(unit='iB', unit_scale=True, desc=f'正在从{warc_file_path}构建{quality_label}数据集') as progress_bar:
            texts = []
            valid_count = 0
            for record in ArchiveIterator(warc_file):
                if record.record_type == WarcRecordType.response and record.http_content_type == 'text/html':
                    ...
                    if text and not text.isspace():
                        if not judge_high_quality(text):
                            continue
...
def build_classifier_dataset(output_base_name: str, output_suffix: str, max_sample: Optional[int] = None):

    wiki_warc_path = 'data/wiki/subsampled_positive_500_pages.warc.gz'
    ...
    output_file_path = f"{output_base_name}_{total_count}.{output_suffix}"
...
if __name__ == "__main__":
    build_classifier_dataset(output_base_name = 'data/classifiers_dataset/quality_classifier', output_suffix = 'train', max_sample=500)
