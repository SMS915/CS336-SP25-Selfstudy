...
**不仅仅是完成作业**，本项目旨在以**生产级标准**重构和扩展课程内容。在完成标准教学要求的基础上，重点进行了**底层系统的性能优化（System Optimization）**、**关键架构的消融实验（Ablation Studies）**，并在消费级显卡上成功复现了 **GRPO** 等 SOTA 对齐算法。
...
- **GRPO 算法实现：**摒弃了传统的 Value Model， 实现了**组相对策略优化 (GRPO)**，大幅节省了显存开销，并比对了 **"Dr. GRPO"** 改进策略（去除 Advantage 标准化和响应长度归一化），
