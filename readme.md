...
本仓库包含我在 **Stanford CS336: Language Modeling** 课程中的作业实现代码。该课程涵盖了大型语言模型 (LLM) 的全生命周期，包括从零构建架构、系统优化、数据工程、扩展定律 (Scaling Laws) 以及后训练对齐 (Post-training Alignment)。
...
| **Assignment 3 **(无法完成，需要斯坦福API) | 扩展：Scaling Laws 与计算预算 | &cross;  |
...
* **核心组件：**
    * 在 TinyStories 数据集上实现 **BPE 分词器**（训练与编码/解码）[cite: 3360]。
    * 构建 **Transformer 架构**组件：Embeddings, RMSNorm, SwiGLU 激活函数, RoPE (旋转位置编码), 以及因果多头自注意力机制 (Causal Multi-Head Self-Attention) 
    * 实现训练基础设施：交叉熵损失函数, **AdamW 优化器**, 以及学习率调度器 [cite: 3362]。
    * **文本生成：** 实现解码策略，包括温度缩放 (Temperature scaling) 和 Top-p (Nucleus) 采样 [cite: 4422, 4427]。

### 2. Assignment 2: Systems (优化与并行)
**目标：** 优化 Transformer 的单 GPU 性能并将训练扩展到多 GPU 环境 [cite: 1136, 1138]。

* **核心组件：**
    * **性能分析 (Profiling)：** 使用 Python 工具和 **NVIDIA Nsight Systems** 对前向/后向传播进行基准测试，分析计算和内存瓶颈。
