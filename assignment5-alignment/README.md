...
本项目在单卡4090 (48GB VRAM) 上成功复现了 类 DeepSeek-R1-Zero 的推理能力演进流程 。我们以 **Qwen-2.5-Math-1.5B** 为基座模型，通过将`HuggingFaceH4/Bespoke-Stratos-17k`**“白金数据”监督微调 (SFT)** 与 **Dr.GRPO 强化学习算法**，将 1.5B 参数规模模型的数学推理能力推向了极限。
...
- **全梯度难度覆盖**：
  ...
  - **核心基准 (MATH-500)**：$Pass@1$ 准确率从 Baseline 的 **14.4%** 惊人地跃升至 **83.4%** ；Pass@64 达到 **98.8%**，性能表现追平官方 Instruct 模型 。
...
​	细心的话可以发现，System Prompt中，\</think> 和 \<answer>之间实际上是空格，然而在数据清洗时，所有数据都被洗成了\</think>\n\<answer>（换行分隔）。这构成了一个微小的“指令-数据”语义 GAP。其虽然微小，却是一个探究SFT数据细节之处对模型究竟带来了多大的影响的绝佳切入点。

​	我们设计了一个对照实验：在推理阶段，将 Prompt 修正为与训练数据逻辑一致的**换行符**，试图“帮助”模型消除歧义。实验结果却极具颠覆性：

| 模型 & Prompt         | 指标            | 准确率  | 格式错误率 | 平均长度 | 答案正确平均长度 | 格式错误平均长度 | 现象解读                                                     |
| --------------------- | --------------- | ------- | ---------- | -------- | ---------------- | ---------------- | ------------------------------------------------------------ |
| Baseline & 原版Prompt | MATH500(Pass@1) | 11.4%   | 62.2%      | 445.12   | 160.91           | 615.14           | **基座的脆弱性**：Baseline 极度依赖指令，原版 Prompt 虽有瑕疵但勉强能懂。 |
| Baseline & 改版Prompt | MATH500(Pass@1) | 5.2%(-) | 81.8%      | 441.15   | 147.88           | 501.67           | **指令过敏**：改为换行这个语义分割性极强的分隔符后，Baseline 对 Token 结构的理解完全崩塌，错误率飙升。 |
| SFT & 原版Prompt      | MATH500(Pass@1) | 51.6%   | 36.8%      | 2086.54  | 1135.80          | 3699.17          | **舒适区 (Sweet Spot)**：尽管 Prompt 语义有误，但完全符合训练时的分布特征，性能达到峰值。 |
| SFT & 改版Prompt      | MATH500(Pass@1) | 45.6%   | 41.4%      | 2144.76  | 1084.27          | 3596.65          | **分布偏移 (OOD)**：修正语义反而破坏了特征索引，导致模型犹豫、啰嗦（长度增加），格式控制力下降。 |

##### 洞察：

1. **Baseline 是“听指令的”，SFT 是“背模版的”：**
   Baseline 模型对 Prompt 的变化极其敏感（准确率腰斩），说明它在尝试实时解析指令。

   而 SFT 模型虽然准确率下降，但依然维持了 45% 的高位，且平均长度（~2100）远超 Baseline（~440），说明 SFT 已经内化了 DeepSeek-R1 风格的长思维链模式，这种**“肌肉记忆”**比 Prompt 指令本身更强大。

2. **错误本身就是一种特征 (The Error is the Feature)：**
   对于 SFT 模型而言，Prompt 中那个“错误的空格”不再是一个语义上的 Space，而变成了一个**特定的特征触发锚点 (Feature Anchor)**。

   - 当锚点存在时（原版），模型精准激活“推理模式”。
   - 当锚点改变时（改版），即使改得更符合逻辑，模型也会感到“陌生的困惑”。

3. **熵增带来的性能损耗：**
   观察 **SFT & 改版Prompt** 的数据，其**平均长度 (2144)** 高于原版 (2086)，同时 **格式错误率 (41.4%)** 也高于原版 (36.8%)。这表明当输入分布发生微小偏移（OOD）时，模型的输出**熵（Uncertainty）增加**了——它变得更啰嗦、更犹豫，从而更容易在漫长的生成中犯错。



##### 结论

在小参数模型的 SFT 工程中，**“训练-推理的一致性”拥有最高优先级**。模型并没有在“阅读”你的 Prompt，它只是在进行高维空间的“模式匹配”。因此，**不要试图在推理阶段“修正”训练时的瑕疵**，保持原样才是对模型最大的尊重。
...
| :-----------: | :-----: | :---: | :---: | :-------: | :--------------: | :-------: | :--------: | :----------------------------------------: |
| **GSM8K$^*$** | Pass@1  | 20.9% | 55.2% |   80.4%   |    **80.6%**     |  79.15%   | **85.14%** |    <font color="red">**-4.54%**</font>     |
| **MATH-500**  | Pass@1  | 14.4% | 51.6% |   59.0%   |    **60.6%**     |   55.4%   | **74.60%** |  <span style="color:red">**-14%**</span>   |
...
|               |         |       |       |           |                  |           |            |                                            |
| **MATH-500**  | Pass@64 | 91.6% | 90.6% |   92.6%   |    **93.6%**     |   92.0%   | **93.8%**  |  <span style="color:red">**-0.2%**</span>  |
...
| **AIME 2025** | Pass@64 | 3.3%  | 23.3% |   30.0%   |      30.0%       | **36.7%** | **40.00%** |  <span style="color:red">**-3.3%**</span>  |
