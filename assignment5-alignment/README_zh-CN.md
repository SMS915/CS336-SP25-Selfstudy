...
## 1. 项目概述
本项目旨在探索大语言模型（LLM）的推理能力涌现机制。基于 **Qwen 2.5 Math 1.5B**作为baseline模型，通过 **监督微调 (SFT)** 和 **群体相对策略优化 (GRPO)** 算法，在单张 **NVIDIA vGPU-48GB (4090 48G)** 上成功复现了 DeepSeek R1-Zero 的训练流程。
...
* **Robust Reward**: 在训练循环中注入了鲁棒评分机制，自动修复生成的 `<think><answer>` 间距微瑕以匹配官方评分函数，防止有效推理被误判为 0 分。
...
我们在三个阶段对模型进行了统一标准的评测（Temperature=0.65, Max Tokens=4096, pass@k=1）：
